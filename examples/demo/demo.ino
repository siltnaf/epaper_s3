...
static void drawTopStatusBar()
{
    ...
    portraitFillRect(0, barY, PORTRAIT_WIDTH, barH, 0x00);

    drawHomeStatusIcon(22, barY + 9, 0xFF);
    drawWifiStatusIcon(PORTRAIT_WIDTH - 136, barY + 10, WiFi.status() == WL_CONNECTED, 0xFF);
    drawBatteryStatusIcon(PORTRAIT_WIDTH - 70, barY + 16, getBatterySections(), 0xFF);
}
...
static void drawPortraitTextInRectCentered(const char *text, int32_t rx, int32_t ry, int32_t rw, int32_t rh, const GFXfont *font)
{
    ...
    // Center the full glyph bounds inside the button frame. This keeps digits
    // and operators visually centered instead of aligning by baseline only.
    int32_t cursorX = rx + (rw - w) / 2 - x1;
    int32_t cursorY = ry + (rh - h) / 2 - y1;
    writeln(font, text, &cursorX, &cursorY, textBuffer);
...
static void drawWifiScanningScreen()
{
    memset(framebuffer, 0xFF, EPD_WIDTH * EPD_HEIGHT / 2);
    drawTopStatusBar();
    drawPortraitTextCentered("Scanning WiFi...", 400, (GFXfont *)&FiraSans);
}
...
static void drawSettingsScreen()
{
    ...
    if (!show_password_prompt) {
        ...
        if (scanned_count <= 0) {
            ...
            // Draw a neat "Retry" button
            portraitDrawRect(153, 500, 234, 60, 0x00);
            drawPortraitTextInRect("RETRY SCAN", 153, 500, 234, 60, (GFXfont *)&FiraSans);
        } else {
            // Draw list of networks (up to 10)
            int num_items = scanned_count > 10 ? 10 : scanned_count;
            for (int i = 0; i < num_items; ++i) {
                int y = 140 + i * 65;
                portraitDrawRect(34, y, 472, 55, 0x00);
                drawPortraitTextInRect(scanned_ssids[i], 54, y, 432, 55, (GFXfont *)&FiraSans);
            }
        }
    } else {
